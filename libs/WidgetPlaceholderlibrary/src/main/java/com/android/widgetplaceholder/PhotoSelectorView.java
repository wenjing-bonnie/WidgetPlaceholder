...
public class PhotoSelectorView extends View {
    ...
    public void notifyDataSetChanged(List<Uri> uris) {
        notifyDataSetChanged(uris, false);
    }

    /**
     * 传入是一个集合
     *
     * @param uris    需要更新的Uri的集合
     * @param isClear 是否清空集合中已有的Uri
     */
    public void notifyDataSetChanged(List<Uri> uris, boolean isClear) {

        //mDataSetObservable.notifyChanged();
        //理论上这个情况应该不存在，因为初始化的时候已经相应的赋值
        if (selectorPathList.size() == 0) {
            return;
        }
        //说明传入的集合为空,直接清空UI界面即可
        if (uris == null) {
            uris = new ArrayList<>();
        }
        if (DEBUG) {
            Log.w(TAG, "notifyDataSetChanged 添加的集合的size is " + uris.size() + " , isClear = " + isClear + ", 原集合中已经存在的size is " + selectorPathList.size());
        }
        //如果是reset所有的值，则直接清空，重新添加即可
        if (isClear) {
            selectorPathList.clear();
            selectorPathList.addAll(uris);
            //更新UI
        ...
        //如果设置了最大值，则只放置最大值的个数
        if (maxNumber > 0 && paths.size() > maxNumber) {
            for (int i = 0; i < maxNumber; i++) {
                selectorPathList.add(paths.get(i));
            }
        } else {
    ...
    /**
     * 获取所有新增的View的个数:
     * 开始的时候selectorPathList不含有默认的uri，会自动更新完imageview之后，
     * 在selectorPathList中添加累加默认值.
     */
    private void updateAllAddChildView() {
        List<Uri> paths = selectorPathList;
        if (DEBUG) {
            Log.v(TAG, "updateAllAddChildView 即将要刷新的路径的集合的size为：" + selectorPathList.size() + " , 之前的childViewList size 为 " + childViewList.size());
        }
        ...
        for (int i = 0; i < childSize; i++) {
            //此时里面就是有路径的ImageView了
            childViewList.add(getChildView(paths.get(i), i, false));
        }
        //如果没有超出最大值，则在后面累加可以添加图片的button
        if ((maxNumber > 0 && childSize < maxNumber) || maxNumber <= 0) {
            addDefaultImageView(childSize);
        }
        //更新行数
        if (childViewList.size() % mNumColumns == 0) {
            mNumRow = childViewList.size() / mNumColumns;
        } else {
        ...
        requestLayout();
        if (DEBUG) {
            Log.v(TAG, String.format("updateAllAddChildView selector path size  = %d , child size = %d", selectorPathList.size(), childViewList.size()));
        }
    }
